...
def generate_chunk(cx, cy):
    ...
    for x in range(CHUNK_SIZE):
        for y in range(CHUNK_SIZE):
            r = random.random()
            if r < 0.01:
                objects.append({"type": "tree", "x": x, "y": y})
            elif r < 0.015:
                objects.append({"type": "rock", "x": x, "y": y})
            elif r < 0.018:
                objects.append({"type": "mushroom", "x": x, "y": y})
            elif r < 0.02:
                objects.append({"type": "crystal", "x": x, "y": y})
    return objects
...
while running:
    ...
    for dx in range(-1, VISIBLE_CHUNKS_X):
        for dy in range(-1, VISIBLE_CHUNKS_Y):
            ...
            for obj in tiles:
                ...
                if image:
                    image = pygame.transform.scale(image, (TILE_SIZE*0.8, TILE_SIZE))
    ...
    for dx in range(-3, 4):
        for dy in range(-3, 4):
            ...
            for obj in tiles:
                wx = chunk_x * CHUNK_SIZE + obj["x"]
                wy = chunk_y * CHUNK_SIZE + obj["y"]
                ...
                color = OBJECT_TYPES.get(obj["type"], (255, 255, 255))  # fallback: fehér
